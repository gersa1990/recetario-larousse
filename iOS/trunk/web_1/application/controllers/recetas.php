...
class Recetas extends CI_Controller {
	...
	public function eliminarRelacion($id1 , $id2)
	{
		$delete = $this->recetas_model->deleteRelation($id1,$id2);
		redirect(base_url().'recetas/relationships/'.$id2.'', 'refresh');
	}

	public function view($id){
		$this->load->helper('url');
		$data['recetas_item'] = $this->recetas_model->get_recetas($id);
		$data['relations'] = $this->recetas_model->getRelations($id);
	...
	public function create(){
		...
		if ($this->form_validation->run() === FALSE){
			...
			redirect(base_url().'recetas/relationships/'.$id.'', 'refresh');
		}
	}

	public function getRecipeToRelation($id)
	{
		$titulo = $_POST['text'];
		$recetas = $this->recetas_model->get_recetasBySearch($titulo,$id);
	...
	public function updateR(){
		...
		if ($this->form_validation->run() === FALSE){
			...
			$actualizar = $this->recetas_model->update_recetas($id, $titulo, $id_categoria, $id_app, $procedimiento, $ingredientes, $preparacion, $coccion, $costo, $video, $foto, $user_fav);
